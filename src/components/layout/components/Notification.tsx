...
import { getAllNotifications, updateNotifications } from '../../../services/notifications.service';
...
const Notification = () => {
    ...
    const loadData = async () => {
        ...
        if(response.docs) {
            ...
            const unread_messages = response.docs?.filter((item: any) => item?.status === 0);
...
const notiMenu = (dropdownRef: any, showMenuNoti: any, dataNoti: any) => {
    ...
    return (
        <div ref={dropdownRef} className={`action-wrapper z-20 w-full max-w-sm bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-800 dark:divide-gray-700 ${showMenuNoti ? "" : "hidden"}`} style={{position: 'absolute', right: -13, width: '24rem', top: 32}}>
            ...
            <div className="divide-y divide-gray-100 dark:divide-gray-700" style={{height: 400, overflow: 'auto'}}>
                    ...
                    return (
                        ...
                        >
                            <div className="flex-shrink-0">
                                <div className="rounded-full" style={{backgroundColor: '#25c2e3', width: 40, height: 40}}>
                                    <div
                                        style={{color: 'white', fontSize: 22, marginTop: 7, marginLeft: 6, paddingTop: 4}}
                                    >
                                        {imageUser(item)}
                                    </div>
                                </div>
                            ...
                            <div className="w-full ps-3">
                                ...
                                <div className="text-gray-500 text-sm mb-1.5 dark:text-gray-400">{item?.content}</div>
