...
type TotalOrderByPrescriptionProps = {
  datarderByPrescription?: any[];
};
...
const TotalOrderByPrescription: React.FC<TotalOrderByPrescriptionProps> = ({
  datarderByPrescription: dataPrev,
}) => {
  ...
  const fetch = useCallback(({ from, to }: { from: string; to: string }) => {
    setLoading(true);
    statisticTotalPrescription({ from, to })
      .then((r) => {
        startTransition(() => {
          setData(r ?? []);
        });
...
export default TotalOrderByPrescription;
